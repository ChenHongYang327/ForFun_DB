...
public class MessageController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("getAll")) {
			...
			List<Message> messages = messageService.selectByMemberId(chatroomId, memberId);

			if (messages.size() > 0 && messages != null) {
				int messageReadType = 0;
				for (Message message : messages) {
					messageReadType = messageService.updateRead(message.getMsgId());
				}
				if (messageReadType > 0) {
					int otherMemberId;
					if (memberId == chatRoomService.seleteById(chatroomId).getMemberId1()) {
						otherMemberId = chatRoomService.seleteById(chatroomId).getMemberId2();
					} else {
						otherMemberId = chatRoomService.seleteById(chatroomId).getMemberId1();
					}
						registrationToken = memberService.selectById(otherMemberId).getToken();
